module retire (
    ...
    input  wire [31:0] i_ex_addr        ,       // 异常地址
    ...
    assign o_ex_sign    = i_ex_sign;
    assign o_ex_pc      = i_ex_sign ? i_pc : 32'd0;
    assign o_ex_addr    = i_ex_sign ? (choose_addr ? i_ex_addr : i_pc) : 32'd0;
...
// 以下皆为正常提交时输出**************************************************
    ...
    assign o_grf_wdata = valid ? i_grf_wdata : 32'd0;

    // ----- CSR 写回 -----
    assign o_csr_we    = valid ? i_csr_we    : 1'b0;
    ...
    // ----- LLbit（提交时直接更新，不受异常屏蔽）-----
    assign o_llbit_set = valid ? i_op_lsu[5]                 : 1'b0;    // 原子访存都会改
    assign o_llbit     = valid ? (i_op_lsu[5] & i_op_lsu[3]) : 1'b0;    // LL指令置为1，SC指令置为0
    ...
    assign o_cacop_sign = valid ? (o_llbit_set | o_csr_we | o_idle_sign | inst_tlb | invtlb_en) : 1'b0;     // 只要是tlb指令就会改csr，直接冲刷不再暂停
    ...
    assign o_actual_target = need_feedback ? i_actual_target : 32'd0;
    ...
    wire flush = o_ex_sign || brach_error || o_cacop_sign || o_ertn_sign;
